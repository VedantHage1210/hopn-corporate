...
<section style="position:relative; overflow:hidden; background:#030712; padding:80px 0 60px;">
    ...
    <div style="position:absolute; top:-100px; right:-100px; width:600px; height:600px; border-radius:50%; background:radial-gradient(circle, rgba(79,110,247,0.06) 0%, transparent 70%); pointer-events:none;"></div>

    <div class="container-shell" style="position:relative; z-index:10;">
        <div style="max-width:700px;">
            <div style="display:inline-flex; align-items:center; gap:8px; bord<x-layouts.public :title="$lang==='ar'?'غرفة الأخبار':($lang==='de'?'Newsroom':'Newsroom')">
...
</x-layouts.public>er:1px solid rgba(79,110,247,0.3); background:rgba(79,110,247,0.08); border-radius:999px; padding:6px 18px; margin-bottom:24px;">
                <span style="width:6px; height:6px; border-radius:50%; background:#4F6EF7; display:inline-block; box-shadow:0 0 8px #4F6EF7;"></span>
                <span style="font-size:11px; font-weight:700; letter-spacing:0.14em; text-transform:uppercase; color:#818CF8;">
                    @if($lang==='ar') غرفة الأخبار @elseif($lang==='de') Newsroom @else Newsroom @endif
                </span>
            </div>
            <h1 style="font-size:clamp(32px,5vw,64px); font-weight:900; color:white; line-height:1.05; letter-spacing:-2px; margin-bottom:20px;">
                @if($lang==='ar') آخر الأخبار والرؤى من HOPn
                @elseif($lang==='de') Neuigkeiten & Einblicke von HOPn
                @else Latest News & Insights from HOPn @endif
            </h1>
            <p style="font-size:17px; color:#64748B; line-height:1.7; max-width:560px;">
                @if($lang==='ar') إعلانات وشراكات وتحديثات المنتجات وأنشطة البحث والذكاء الاصطناعي.
                @elseif($lang==='de') Ankündigungen, Partnerschaften, Produktupdates und Forschung von HOPn.
                @else Announcements, partnerships, product updates, AI research, and innovation stories from HOPn. @endif
            </p>
        </div>
    </div>
</section>
...
{{-- Categories Filter --}}
<section style="background:#030712; border-bottom:1px solid rgba(255,255,255,0.05); position:sticky; top:56px; z-index:40;">
    <div class="container-shell">
        <div style="display:flex; gap:0; overflow-x:auto; padding:0;">
            @php
            $categories = [
                ['key'=>'all',          'en'=>'All',          'de'=>'Alle',          'ar'=>'الكل'],
                ['key'=>'ai',           'en'=>'AI',           'de'=>'KI',            'ar'=>'الذكاء الاصطناعي'],
                ['key'=>'data',         'en'=>'Data',         'de'=>'Daten',         'ar'=>'البيانات'],
                ['key'=>'products',     'en'=>'Products',     'de'=>'Produkte',      'ar'=>'المنتجات'],
                ['key'=>'startups',     'en'=>'Startups',     'de'=>'Startups',      'ar'=>'الشركات الناشئة'],
                ['key'=>'funding',      'en'=>'Funding',      'de'=>'Finanzierung',  'ar'=>'التمويل'],
                ['key'=>'partnerships', 'en'=>'Partnerships', 'de'=>'Partnerschaften','ar'=>'الشراكات'],
                ['key'=>'events',       'en'=>'Events',       'de'=>'Events',        'ar'=>'الفعاليات'],
                ['key'=>'research',     'en'=>'Research',     'de'=>'Forschung',     'ar'=>'البحث'],
                ['key'=>'robotics',     'en'=>'Robotics',     'de'=>'Robotik',       'ar'=>'الروبوتيكا'],
            ];
            @endphp
            @foreach($categories as $cat)
            <a href="?category={{ $cat['key'] }}"
               style="display:inline-flex; align-items:center; white-space:nowrap; padding:16px 20px; font-size:13px; font-weight:600; text-decoration:none; border-bottom:2px solid {{ $activeCategory===$cat['key']?'#4F6EF7':'transparent' }}; color:{{ $activeCategory===$cat['key']?'white':'#475569' }}; transition:all 0.2s;"
               onmouseover="this.style.color='white'"
               onmouseout="if('{{ $activeCategory }}' !== '{{ $cat['key'] }}') this.style.color='#475569'">
                {{ $cat[$lang] ?? $cat['en'] }}
            </a>
            @endforeach
        </div>
...
<section style="padding:60px 0 100px; background:#030712;">
    <div class="container-shell">
        ...
        <a href="{{ route('insights.show', ['lang'=>$lang,'slug'=>$featured->slug]) }}"
           style="display:grid; grid-template-columns:1fr 1fr; gap:0; border:1px solid rgba(255,255,255,0.06); background:#0A0F1E; border-radius:20px; overflow:hidden; text-decoration:none; margin-bottom:32px; transition:all 0.3s;"
           onmouseover="this.style.borderColor='rgba(79,110,247,0.3)'; this.style.background='#0D1425'"
           onmouseout="this.style.borderColor='rgba(255,255,255,0.06)'; this.style.background='#0A0F1E'">
            <div style="padding:40px; display:flex; flex-direction:column; gap:20px; justify-content:center;">
                <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
                    @if($featured->category)
                    <span style="font-size:11px; font-weight:700; padding:4px 12px; border-radius:999px; background:rgba(79,110,247,0.1); border:1px solid rgba(79,110,247,0.2); color:#818CF8; text-transform:uppercase;">
                        {{ $featured->category->name ?? 'News' }}
                    </span>
                    @endif
                    <span style="font-size:11px; font-weight:700; padding:4px 12px; border-radius:999px; background:rgba(16,185,129,0.1); border:1px solid rgba(16,185,129,0.2); color:#10B981;">
                        @if($lang==='ar') مميز @elseif($lang==='de') Highlight @else Featured @endif
                    </span>
                </div>
                <h2 style="font-size:clamp(20px,3vw,32px); font-weight:800; color:white; line-height:1.3; margin:0;">
                    @if($lang==='de'&&$featured->title_de) {{ $featured->title_de }}
                    @elseif($lang==='ar'&&$featured->title_ar) {{ $featured->title_ar }}
                    @else {{ $featured->title }} @endif
                </h2>
                ...
                <div style="display:flex; align-items:center; gap:16px; flex-wrap:wrap;">
                    ...
                    <span style="font-size:13px; font-weight:600; color:#4F6EF7;">
                        @if($lang==='ar') اقرأ المزيد @elseif($lang==='de') Mehr lesen @else Read more @endif →
                    </span>
                </div>
            </div>
            <div style="background:#0D1425; min-height:280px; display:flex; align-items:center; justify-content:center; overflow:hidden;">
                @if($featured->featured_image_path)
                <img src="{{ $featured->featured_image_path }}" alt="{{ $featured->title }}"
                ...
                <div style="text-align:center; color:#1e293b;">
                    ...
                    <div style="font-size:13px; font-weight:600; color:#334155;">HOPn Newsroom</div>
                </div>
                @endif
            </div>
        </a>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(300px,1fr)); gap:20px;">
            ...
            <a href="{{ route('insights.show', ['lang'=>$lang,'slug'=>$post->slug]) }}"
               ...
               onmouseout="this.style.borderColor='rgba(255,255,255,0.06)'; this.style.background='#0A0F1E'; this.style.transform='translateY(0)'">
                ...
                @else
                <div style="height:80px; background:linear-gradient(135deg,{{ $c }}15,{{ $c }}05); display:flex; align-items:center; justify-content:center;">
                ...
                <div style="padding:20px; display:flex; flex-direction:column; flex:1; gap:10px;">
                    @if($post->category)
                    <span style="display:inline-block; font-size:11px; font-weight:700; padding:3px 10px; border-radius:999px; background:{{ $c }}10; border:1px solid {{ $c }}20; color:{{ $c }}; width:fit-content; text-transform:uppercase;">
                        {{ $post->category->name ?? 'News' }}
                    </span>
                    @endif
                    <h3 style="font-size:16px; font-weight:700; color:white; line-height:1.4; flex:1; margin:0;">
                        @if($lang==='de'&&$post->title_de) {{ $post->title_de }}
                        @elseif($lang==='ar'&&$post->title_ar) {{ $post->title_ar }}
                        @else {{ $post->title }} @endif
                    </h3>
                    ...
                    <p style="font-size:13px; color:#475569; line-height:1.6; margin:0;">
                        @if($lang==='de'&&$post->excerpt_de) {{ Str::limit($post->excerpt_de,80) }}
                        @elseif($lang==='ar'&&$post->excerpt_ar) {{ Str::limit($post->excerpt_ar,80) }}
                        @else {{ Str::limit($post->excerpt,80) }} @endif
                    </p>
                    @endif
                    <div style="display:flex; align-items:center; justify-content:space-between; padding-top:12px; border-top:1px solid rgba(255,255,255,0.05);">
                        <span style="font-size:12px; color:#334155;">
        ...
        @else
        <div style="text-align:center; padding:100px 40px; color:#334155;">
            <div style="font-size:64px; margin-bottom:20px;">📰</div>
            <h3 style="font-size:24px; font-weight:800; color:#475569; margin-bottom:12px;">
                @if($lang==='ar') لا توجد مقالات بعد @elseif($lang==='de') Noch keine Artikel @else No Articles Yet @endif
            </h3>
            <p style="font-size:16px; color:#334155;">
                @if($lang==='ar') أضف مقالات من لوحة الإدارة.
                @elseif($lang==='de') Artikel über das Admin-Panel hinzufügen.
                @else Add articles from the admin panel. @endif
            </p>
        </div>
        @endif
    </div>
