    ...
    <div class="container-shell hopn-reveal" style="position:relative; z-index:10;">
        <div style="display:flex; justify-content:space-between; align-items:flex-end; flex-wrap:wrap; gap:20px; margin-bottom:40px;">
            <div>
                ...
                <h1 style="font-size:clamp(28px,5vw,56px); font-weight:900; color:white; letter-spacing:-2px; margin:0; line-height:1.1;">
                    @if($lang==='ar') آخر الأخبار والإعلانات @elseif($lang==='de') Neuigkeiten & Ankündigungen @else Latest News &amp; Announcements @endif
                </h1>
                <p style="font-size:16px; color:#CBD5E1; margin-top:12px; line-height:1.7; max-width:500px;">
                    @if($lang==='ar') الإعلانات والشراكات وتحديثات المنتجات وأبحاث الذكاء الاصطناعي وقصص الابتكار من HOPn.
                    @elseif($lang==='de') Ankündigungen, Partnerschaften, Produktupdates, KI-Forschung und Innovationsgeschichten von HOPn.
                    @else Announcements, partnerships, product updates, AI research, and innovation stories from HOPn. @endif
                </p>
            </div>
            {{-- Search --}}
            <form method="GET" action="{{ route('newsroom.index', ['lang'=>$lang]) }}" style="display:flex; gap:10px;">
                <div style="position:relative;">
                    ...
                    <input type="text" name="search" value="{{ $search ?? '' }}"
                           placeholder="{{ $lang==='ar'?'بحث...':($lang==='de'?'Suchen...':'Search news...') }}"
        ...
        <div style="display:flex; flex-wrap:wrap; gap:8px; border-bottom:1px solid rgba(255,255,255,0.06); padding-bottom:0;">
            <a href="{{ route('newsroom.index', ['lang'=>$lang]) }}"
               style="padding:10px 18px; font-size:13px; font-weight:600; text-decoration:none; border-bottom:2px solid {{ !$category||$category==='all'?'#4F6EF7':'transparent' }}; color:{{ !$category||$category==='all'?'white':'#CBD5E1' }}; transition:all 0.2s; margin-bottom:-1px;">
                @if($lang==='ar') الكل @elseif($lang==='de') Alle @else All @endif
            </a>
            @foreach($categories as $cat)
            <a href="{{ route('newsroom.index', ['lang'=>$lang, 'category'=>$cat->slug]) }}"
               class="hopn-link-accent" style="padding:10px 18px; font-size:13px; font-weight:600; text-decoration:none; border-bottom:2px solid {{ $category===$cat->slug?'#4F6EF7':'transparent' }}; color:{{ $category===$cat->slug?'white':'#CBD5E1' }}; transition:all 0.2s; margin-bottom:-1px; white-space:nowrap;">
                {{ $cat->name }}
...
{{-- FEATURED POST --}}
@if($featured && (!$category || $category==='all') && !$search)
           ...
           class="hopn-lift-btn" style="display:grid; grid-template-columns:1fr 1fr; gap:0; border:1px solid rgba(255,255,255,0.07); background:#0A0F1E; border-radius:20px; overflow:hidden; text-decoration:none; transition:all 0.3s;">
            <div style="position:relative; min-height:300px; background:linear-gradient(135deg,rgba(79,110,247,0.2),rgba(139,92,246,0.2)); display:flex; align-items:center; justify-content:center; overflow:hidden;">
                ...
                <div style="position:absolute; top:16px; left:16px;">
                    <span style="font-size:11px; font-weight:700; padding:4px 12px; border-radius:999px; background:rgba(16,185,129,0.9); color:white; text-transform:uppercase;">
            ...
            <div style="padding:40px; display:flex; flex-direction:column; justify-content:center; gap:16px;">
                ...
                @endif
                <h2 style="font-size:clamp(20px,3vw,30px); font-weight:800; color:white; line-height:1.3; margin:0;">
...
<section style="padding:40px 0 100px; background:#030712;">
    <div class="container-shell">
        ...
        <div style="margin-bottom:24px; color:#CBD5E1; font-size:14px;">
            ...
            <a href="{{ route('newsroom.index', ['lang'=>$lang]) }}" style="margin-left:12px; color:#EF4444; font-size:12px;">✕ Clear</a>
        </div>
        @endif
