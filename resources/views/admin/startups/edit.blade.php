        ...
        <form method="POST" action="{{ route('admin.startups.update', $startup) }}">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="url" name="logo" value="{{ old('logo', $startup->logo ?? '') }}"
                           placeholder="https://example.com/logo.png"
                           class="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-white">
                    @if(!empty($startup->logo))
                    <div class="mt-2">
                ...
                <div>
                    ...
                    <input type="text" name="name_de" value="{{ old('name_de', $startup->name_de ?? '') }}"
                           class="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-white">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-400 mb-1">Name (AR)</label>
                    <input type="text" name="name_ar" value="{{ old('name_ar', $startup->name_ar ?? '') }}" dir="rtl"
                           class="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-white">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="industry_ar" value="{{ old('industry_ar', $startup->industry_ar ?? '') }}" dir="rtl"
                           class="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-white">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4"
                              class="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-white">{{ old('description', $startup->description) }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-400 mb-1">Description (DE)</label>
                    <textarea name="description_de" rows="4"
                              class="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-white">{{ old('description_de', $startup->description_de ?? '') }}</textarea>
                </div>
                <div>
                    ...
                    <textarea name="description_ar" rows="4" dir="rtl"
                              class="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-white">{{ old('description_ar', $startup->description_ar ?? '') }}</textarea>
                </div>

                {{-- Visibility --}}
                <div class="md:col-span-2 flex items-center gap-2 mt-2">
                    <input type="checkbox" name="is_visible" id="is_visible" value="1"
                           {{ old('is_visible', $startup->is_visible ?? true) ? 'checked' : '' }}>
