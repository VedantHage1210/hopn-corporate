    ...
    <form method="GET" class="mb-6 flex flex-wrap gap-3 items-end">
        <div>
            ...
            <select name="type" class="rounded border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white">
                ...
                <option value="training-application" @selected(request('type') === 'training-application')>Training Application</option>
                <option value="event-registration" @selected(str_starts_with(request('type', ''), 'event-registration'))>Event Registration</option>
                <option value="startup-application" @selected(request('type') === 'startup-application')>Startup Application</option>
                <option value="career-application" @selected(request('type') === 'career-application')>Career Application</option>
            </select>
        </div>
        <div>
        ...
        <table class="min-w-full text-sm text-slate-300">
            ...
            <tbody>
                ...
                @php
                    ...
                    $typeColors = [
                        'contact'              => 'bg-slate-700 text-slate-300',
                        'proposal'             => 'bg-indigo-900 text-indigo-200',
                        'partner-inquiry'      => 'bg-cyan-900 text-cyan-200',
                        'training-application' => 'bg-amber-900 text-amber-200',
                    ...
                    $typeLabel = str_starts_with($lead->type, 'event-registration')
                        ? 'Event: ' . trim(str_replace('event-registration:', '', $lead->type))
