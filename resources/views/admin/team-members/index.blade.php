<x-layouts.admin title="Team Members">
    <div class="mb-6 flex items-center justify-between">
        ...
        <a href="{{ route('admin.team-members.create') }}" class="btn-primary text-sm">+ New Member</a>
    </div>
    @if(session('status'))
        ...
        <table class="min-w-full text-sm text-slate-300">
            <thead class="text-left text-xs uppercase text-slate-400">
                <tr>
                    ...
                    <th class="px-3 py-2">Role (EN)</th>
                    <th class="px-3 py-2">Role (DE)</th>
                    <th class="px-3 py-2">Role (AR)</th>
            ...
            <tbody>
                ...
                <tr class="border-t border-slate-800 hover:bg-slate-800/30">
                    <td class="px-3 py-3 text-slate-400">{{ $member->id }}</td>
                    <td class="px-3 py-3">
                        @if($member->photo)
                            <img src="{{ $member->photo }}" alt="{{ $member->name }}"
                                 class="h-10 w-10 rounded-full object-cover">
                        @else
                            <div class="h-10 w-10 rounded-full bg-indigo-900 flex items-center justify-center text-xs font-bold text-indigo-300">
                    ...
                    <td class="px-3 py-3">
                        <span class="rounded-full px-2 py-0.5 text-xs {{ $member->visible ? 'bg-green-900 text-green-200' : 'bg-slate-700 text-slate-400' }}">
                ...
                @empty
                <tr><td colspan="9" class="px-3 py-6 text-center text-slate-500">No team members found.</td></tr>
                @endforelse
            </tbody>
        </table>
