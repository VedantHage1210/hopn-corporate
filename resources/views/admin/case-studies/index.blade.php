<x-layouts.admin title="Case Studies">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-xl font-semibold text-white">Case Studies</h1>
                  ...
                  <td class="px-3 py-3 text-slate-400 text-xs">{{ $loop->iteration }}</td>
                    ...
                    <td class="px-3 py-3">
                        <div class="flex gap-3">
                            <a href="{{ route('admin.case-studies.edit', $item) }}" class="text-indigo-300 hover:text-indigo-200">Edit</a>
                            <a href="{{ url('/en/case-studies/'.$item->slug) }}" target="_blank" class="text-slate-400 hover:text-white">View</a>
                            @can('content.delete')
<form method="POST" action="{{ route('admin.case-studies.destroy', $item) }}" class="inline-block">
                                @csrf @method('DELETE')
...
@endcan
                ...
                @empty
                <tr><td colspan="6" class="px-3 py-6 text-center text-slate-500">No case studies found.</td></tr>
                @endforelse
            </tbody>
        </table>
