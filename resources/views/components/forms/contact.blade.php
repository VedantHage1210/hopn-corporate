@props(['lang' => 'en'])
<form action="{{ route('contact.submit', ['lang' => $lang]) }}" method="POST" class="space-y-4">
    @csrf
    <input type="text" name="honeypot" style="display:none">
    @if(session('contact_status'))
        <div class="rounded-lg bg-green-900/40 border border-green-700 px-4 py-3 text-sm text-green-300">{{ session('contact_status') }}</div>
    @endif
    <div>
    ...
    <div>
        ...
        <input type="text" name="phone" value="{{ old('phone') }}" pattern="[0-9+\-\s()]*" inputmode="tel" title="{{ $lang === 'ar' ? 'يُسمح فقط بالأرقام والمسافات و + و - و ()' : ($lang === 'de' ? 'Nur Zahlen, Leerzeichen, +, - und () sind erlaubt.' : 'Only numbers, spaces, +, -, and () are allowed.') }}"
            class="w-full rounded border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white focus:border-indigo-500 focus:outline-none">
    ...
    <div class="flex items-start gap-2">
        <input type="checkbox" name="gdpr" id="gdpr_contact" value="1" class="mt-1" required>
        <label for="gdpr_contact" class="text-xs text-slate-400">{{ $lang === 'ar' ? 'أوافق على سياسة الخصوصية وعلى التواصل معي.' : ($lang === 'de' ? 'Ich stimme der Datenschutzerklärung zu.' : 'I agree to the Privacy Policy and consent to being contacted.') }}</label>
    </div>
    @error('gdpr')<p class="text-xs text-rose-300">{{ $message }}</p>@enderror
    <button type="submit" class="btn-primary w-full">{{ $lang === 'ar' ? 'إرسال الرسالة' : ($lang === 'de' ? 'Nachricht senden' : 'Send Message') }}</button>
</form>
