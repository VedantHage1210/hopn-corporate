...
<head>
    ...
    <style>
        ...
        #toggle-sidebar svg { width:24px; height:24px; }
        ...
        .sidebar-open aside { display:block !important; position:fixed !important; left:0 !important; top:0 !important; height:100vh !important; z-index:50 !important; width:256px !important; overflow-y:auto !important; }
        .close-sidebar-btn { background:none; border:none; cursor:pointer; padding:0; color:#cbd5e1; font-size:24px; display:none; }
        .sidebar-open .close-sidebar-btn { display:block; }
        .sidebar-open .close-sidebar-btn svg { width:24px; height:24px; }
        @media (min-width:768px) {
            aside { display:block !important; position:static !important; height:auto !important; width:auto !important; }
...
<a class="block hover:text-white transition-colors duration-200" href="{{ route('admin.authors.index') }}">Authors</a>
                ...
                <a class="block hover:text-white transition-colors duration-200" href="{{ route('admin.partners.index') }}">Logos & Partners</a>
   ...
   View Catalog
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (toggleBtn) { toggleBtn.addEventListener('click', function() { appContainer.classList.add('sidebar-open'); }); }
            if (closeBtn) { closeBtn.addEventListener('click', function() { appContainer.classList.remove('sidebar-open'); }); }
            if (overlay) { overlay.addEventListener('click', function() { appContainer.classList.remove('sidebar-open'); }); }
            document.querySelectorAll('aside a').forEach(link => { link.addEventListener('click', function() { appContainer.classList.remove('sidebar-open'); }); });
        });
    </script>
</body>
</html>